...
def insert_sample_data(conn):
    ...
    sample_incidents = [
        ('Mass Casualty Event', 'Multiple casualties arriving from highway accident', datetime.utcnow().isoformat(), 'Critical', 'Open'),
        ('Power Outage', 'Hospital lost main power, on backup generator', datetime.utcnow().isoformat(), 'High', 'In Progress'),
        ('Supply Shortage', 'Running low on sterile gloves in Emergency', datetime.utcnow().isoformat(), 'Medium', 'Open'),
        ('Staffing Issue', 'Shortage of nurses in ICU for night shift', datetime.utcnow().isoformat(), 'High', 'Open'),
    ]
...
with tab4:
    ...
    if not tasks_df.empty:
        ...
        task_to_update = st.selectbox("Select Task to Update", options=task_options, format_func=lambda x: x[0] if isinstance(x, tuple) else x)

        if task_to_update:
            current_task_info = tasks_df[tasks_df["id"] == task_to_update[1]].iloc[0]
            ...
            if st.button("Update Task Status"):
                conn.execute("UPDATE tasks SET status=?, timestamp=? WHERE id=?",
                             (new_status, datetime.utcnow().isoformat(), task_to_update[1]))
                conn.commit()
